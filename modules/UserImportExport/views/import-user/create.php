        ...
        for (var index = 0; index < files.length; index++) {
            ...
            $.ajax({
                ...
                if (response.uploaded) {
                    $("#import_upload_id").val(response.model.id);
                    $("#form").submit();
                } else {
...
<?= $form->field($model, 'step')->hiddenInput(['id' => 'import_step', 'onChange' => 'handleFile(this)'])->label(false); ?>
...
<?php if ($model->step == 1): ?>
    ...
    <?= $form->field($model, 'upload_id')->widget(
        ...
        [
            ...
            'items' =>  yii\helpers\ArrayHelper::map(\app\models\Upload::find()->all(), 'id', function ($item) {
